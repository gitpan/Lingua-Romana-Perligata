...
(
	...
	'decapita'	=> { perl => 'shift' },
	'claude'	=> { perl => 'close' },
	'perlege'	=> { perl => 'IO::Handle::getline' },
	'sublege'	=> { perl => 'getc' },
	'enunta'	=> { perl => 'tell' },
	'dele'		=> { perl => 'delete' },
	'quisque'	=> { perl => 'each' },
...
sub tokenize
{
	...
	while (length $text)
	{
		...
		elsif ($text =~ s/$tokensque//)
		{
			...
			push @tokens, tokdup $tokens{$1};
		}
		elsif ($text =~ s/$tokensve//)
		{
		...
		{
			push @tokens, tokdup $connectives{$4} if $4;
			my $perl = $3 ? "\$$2->" : $2;
		...
		{
			push @tokens, tokdup $connectives{$3} if $3;
		...
		{
			push @tokens, tokdup $connectives{$2} if $2;
...
sub Literal::translate {
	...
	if ($context && $context->{raw} =~ /^inque/) {
		...
		return $self->GENITIVE::translate;
	}
	elsif ( $self->{lex} eq 'SUBNAME_A_GENITIVE' ) {
		return $self->SUBNAME_A_GENITIVE::translate;
	}
...
C<$hoh{key}{subkey}{subsubkey}>, but not $lohol[$index1]{key}[$index2]>.
...
The first ten ordinals are named (in the accustive): I<primum>/I<primam>,
